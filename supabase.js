    ...
    window.CardlyAPI = {
      ...
      mapCarteFromDB(c, profile, entreprise) {
        return {
          ...
          afficher_nom:        c.afficher_nom            ?? true,
          afficher_prenom:     c.afficher_prenom         ?? true,
          afficher_entreprise: c.afficher_nom_entreprise ?? true,
          afficher_poste:      c.afficher_poste          ?? true,
          afficher_telephone:  c.afficher_telephone      ?? true,
          ...
          fieldSides: {
            ...
            poste:      c.poste_side          || 'recto',
            phone:      c.telephone_side      || 'recto',
            email:      c.email_side          || 'recto',
            web:        c.site_web_side       || 'recto',
          },
          fieldSizes: {
            name:       (+(c.prenom_size         || 100)) / 100,
            entreprise: (+(c.nom_entreprise_size || 100)) / 100,
            poste:      (+(c.poste_size          || 90))  / 100,
            phone:      (+(c.telephone_size      || 80))  / 100,
            email:      (+(c.email_size          || 80))  / 100,
            web:        (+(c.site_web_size       || 80))  / 100,
          },
          fieldFonts: {
            name:       (c.prenom_police === 'Inter' ? 'default' : c.prenom_police)                 || 'default',
            entreprise: (c.nom_entreprise_police === 'Inter' ? 'default' : c.nom_entreprise_police) || 'default',
            poste:      (c.poste_police === 'Inter' ? 'default' : c.poste_police)                   || 'default',
            phone:      (c.telephone_police === 'Inter' ? 'default' : c.telephone_police)           || 'default',
            email:      (c.email_police === 'Inter' ? 'default' : c.email_police)                   || 'default',
      ...
      carteToDBUpdate(card, { fieldColors = {}, fieldSides = {}, fieldSizes = {}, fieldFonts = {}, fieldDecorations = {}, logoUrl, logoSide, logoSizeRecto, logoSizeVerso, frontImageUrl, backImageUrl } = {}) {
        ...
        return {
          ...
          prenom_x:    card.positions?.name?.x  ?? 50,  prenom_y:    card.positions?.name?.y  ?? 30,
          nom_x:       card.positions?.name?.x  ?? 50,  nom_y:       card.positions?.name?.y  ?? 30,
          poste_x:     card.positions?.poste?.x ?? 50,  poste_y:     card.positions?.poste?.y ?? 42,
          telephone_x: card.positions?.phone?.x ?? 50,  telephone_y: card.positions?.phone?.y ?? 60,
          email_x:     card.positions?.email?.x ?? 50,  email_y:     card.positions?.email?.y ?? 68,
          site_web_x:  card.positions?.web?.x   ?? 50,  site_web_y:  card.positions?.web?.y   ?? 76,
          prenom_couleur:         fc.name       || '#ffffff',
          nom_couleur:            fc.name       || '#ffffff',
          nom_entreprise_couleur: fc.entreprise || '#ffffff',
          poste_couleur:          fc.poste      || '#ffffff',
          telephone_couleur:      fc.phone      || '#ffffff',
          email_couleur:          fc.email      || '#ffffff',
          site_web_couleur:       fc.web        || '#ffffff',
